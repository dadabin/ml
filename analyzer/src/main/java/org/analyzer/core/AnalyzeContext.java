...
public class AnalyzeContext {
    ...
    /**
     * 根据context 的上下文情况，填充segmentBuff
     *
     * @param reader
     * @return 返回待分析的（有效的）字符串长度
     * @throws IOException
     */
    ...
    /**
     * 指针+1 成功返回true，指针已经到了buff尾部，不能前进，返回false 并处理当前字符
     *
     * @return
     */
    boolean moveCursor() {
    ...
    /**
     * 判断segmentBuff是否需要读取新数据 满足一下条件时， 1.available == BUFF_SIZE 表示buffer满载
    ...
    void outputToResult() {
        ...
        for (; index <= this.cursor;) {
            ...
            // 从pathMap找出对应的index位置的LexemePath
    ...
    Lexeme getNextLexeme() {
        ...
        while (result != null) {
            ...
            if (Dic.getSingleton().isStopWord(this.segmentBuff, result.getBegin(),
                ...
                result.setLexemeText(String.valueOf(segmentBuff, result.getBegin(),
                        result.getLength()));
                break;
            }
        }
