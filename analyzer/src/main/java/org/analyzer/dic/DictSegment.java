...
class DictSegment implements Comparable<DictSegment> {
    ...
    private Map<Character, DictSegment> getChildrenMap() {
        if (this.childrenMap == null) {
            synchronized (this) {
                if (this.childrenMap == null) {
                    this.childrenMap = new HashMap<Character, DictSegment>(ARRAY_LENGTH_LIMIT * 2,
                            0.8f);
                }
            }
        }
    ...
    @Override
    public int compareTo(DictSegment arg0) {
        // 对当前节点存储的char进行比较
        return this.nodeChar.compareTo(arg0.nodeChar);
    }
